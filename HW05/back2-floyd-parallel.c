...
/*		if(k==0){
			int x,y;
			printf("Proces %d has subs:\n",rank);
			for(x=0;x<local_rows;x++){
				for(y=0;y<local_cols;y++)
					printf("\t%d",((int**)subs)[x][y]);
				printf("\n");
			}
			printf("Proces %d has rbuf:\n",rank);
			for(x=0;x<local_cols;x++)
				printf("\t%d",rbuf[x]);
			printf("\n");
			printf("Proces %d has cbuf:\n",rank);
			for(x=0;x<local_rows;x++)
				printf("\t%d",cbuf[x]);
			printf("\n");
		}
